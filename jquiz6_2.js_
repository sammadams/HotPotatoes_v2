/*
COMMENTS FOR ANDY:
New edits are mostly contained from lines 88-183:
line 758 StoreResults() sends score variables to cookies after exercise is completed
 ^-- nested inside line 726 CheckFinished()
 
 A lot of this code could probably be cleaned and updated for our purposes but I only had time to implement the cookie storage function that we talked about during TESOL2018.
 I hope we can work on this together! If you think this is something you want to collab on, we can create a GitHub folder to work on the same version.
...
//New for 6.2.2.0
var CompletedSoFar = '[strCompletedSoFar]';
var ExerciseCompleted = '[strExerciseCompleted]';
...
START EDITS
*/
// enable cookie storage capability
	// I took this code from GitHub
	var docCookies = {
...
// end cookie storage
	...
	// counts existing cookies
	cookieString = [];
	// parse cookies individually
	...
	// grabs number from end of string
	// each time an exercise is completed, three cookies are created
	// all three cookies share the same number at the end of the string: "exerciseName_#"; "exerciseScore_#"; "exerciseTime_#"
	// I did this because I was running into problems for generating the table rows based on the number of cookies
	var exerciseCount = Number(lastCookie.substr(lastCookie.length - 1));
	console.log("exercise count is " + exerciseCount);
	
	// generate correct exercise number for activity
	var count = 0;
	...
	// store variables in cookies; grabs count from countExercises() above
...
// call storage function for cookies
		StoreResults();
